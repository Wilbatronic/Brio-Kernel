    ...
    D: AgentDispatcher,
{
    ...
    #[must_use]
    pub const fn new(repository: R, dispatcher: D) -> Self {
        Self {
            repository,
            dispatcher,
        }
    ...
    pub fn poll_pending_tasks(&self) -> Result<u32, SupervisorError> {
        let pending_tasks = self.repository.fetch_pending_tasks()?;
        let mut dispatched_count: u32 = 0;

        for task in pending_tasks {
            match self.dispatch_task(&task) {
                Ok(true) => dispatched_count += 1,
                Ok(false) => { /* Agent busy, task remains pending */ }
                Err(e) => {
                    self.handle_dispatch_failure(&task, &e);
                }
            }
        }
    ...
    /// Returns `Ok(true)` if dispatched, `Ok(false)` if agent was busy.
    fn dispatch_task(&self, task: &Task) -> Result<bool, SupervisorError> {
        ...
        match self.dispatcher.dispatch(&agent, task)? {
            DispatchResult::Accepted => {
                self.repository
                    .mark_assigned(task_id, &agent)
                    .map_err(SupervisorError::StatusUpdateFailure)?;
                Ok(true)
            }
            DispatchResult::AgentBusy => {
                // Leave task as pending for next cycle
                Ok(false)
            }
        }
    }
    ...
    fn handle_dispatch_failure(&self, task: &Task, error: &SupervisorError) {
...
mod tests {
    ...
    fn poll_dispatches_pending_tasks() {
        ...
        let count = supervisor.poll_pending_tasks().unwrap();

        assert_eq!(count, 2);
    }

    #[test]
    fn poll_handles_busy_agents() {
        let repo = MockRepository::new(vec![test_task(1, "task1")]);

        let dispatcher = MockDispatcher {
            result: DispatchResult::AgentBusy,
        };

        let supervisor = Supervisor::new(repo, dispatcher);
        let count = supervisor.poll_pending_tasks().unwrap();

        // Task not dispatched because agent is busy
        assert_eq!(count, 0);
    }

    #[test]
    fn poll_marks_assigned_on_accept() {
        let repo = MockRepository::new(vec![test_task(42, "task42")]);
        ...
        let supervisor = Supervisor::new(repo, dispatcher);
        supervisor.poll_pending_tasks().unwrap();
